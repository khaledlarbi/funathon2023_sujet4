...
### Entrainer son propre classifieur (🔴,⚫)
...
du SSP Cloud. Nous proposons ici d'utiliser le package `s3fs` qui est 
...
⚠️ il faut avoir modifié la valeur de `BUCKET` dans le fichier `config.yaml` pour
...
```{python}
#| eval: false
!mc anonymous set download s3/projet-funathon/2023/sujet4/diffusion
```
