...
En attendant, partir d'un produit standardisé
...
## Association d'un code barre à un produit d'openfoodfacts
...
il peut être utile de faire non seulement de l'apparimeent
...
veut). Ceci est un exercice pour les parcours 🔴 et ⚫, les autres
...
Pour avoir un outil performant, on propose d'utiliser à
`DuckDB` pour lire et filtrer les données. Cela sera plus performant que 
lire, à chaque fois que l'utilisateur de notre applicatoin _upload_ une image,
...
#| output: asis
from utils_notebook import create_box_level
create_box_level(color = "grey", title = "Lire les données avec `DuckDB` (🟢 et 🔵)")
```
<details>
<summary>Dérouler pour révéler les instructions</summary>
...
import numpy as np
import pandas as pd
from utils.preprocess_openfood import clean_note

def fuzzy_matching_product(openfood_produit, product_name, con, url_data, liste_colonnes_sql, info_nutritionnelles, indices_synthetiques):
...
def find_product_openfood(con, liste_colonnes_sql, url_data, ean, coicop):
    ...
    if openfood_produit['energy-kcal_100g'].isnull().iloc[0]:
        openfood_produit = fuzzy_matching_product(openfood_produit, product_name, con, url_data, liste_colonnes_sql, info_nutritionnelles, indices_synthetiques)
...
```{python}
openfood_produit = find_product_openfood(con, liste_colonnes_sql, url_data, ean, coicop)
```
